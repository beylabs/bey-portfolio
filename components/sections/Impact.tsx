    ...
    <section className="py-24 bg-ghost" id="impact">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {PROJECTS.map((proj, i) => {
            const slug = proj.title.toLowerCase().replace(/ /g, '-');
            return (
              <motion.div 
                key={i}
                whileHover={{ y: -10 }}
              ...
              >
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <Link href={`/projects/${slug}`} className="text-center py-2 bg-midnight text-white text-sm font-bold rounded-lg hover:bg-slate-800 transition-colors">
